// I-PAES skeleton program code for the Protein Structure Prediction problem
// (PSP)
/*
...
//    A Multi-Objective Evolutionary Approach to the Protein Structure
//    Prediction Problem.
...
/*********************************************************************************************************
 * To run : *
 *  ./i-paes [Energy] [depth] [objectives] [residues] [archive] [iterations]
 *[Tmax] [protein file]       *
 *           [dir] [SS] *
 *                                                                                                       *
 * where all parameters MUST be specified correctly (none are optional)
 *following the instructions below:*
 *                                                                                                       *
 * [Energy] - at present there is only one Energy function included with this
 *code: CHARMM (v. 27)       *
 * [depth] - this is the number of recursive subdivisions of the objective space
 *carried out in order to *
 *           divide the objective space into a grid for the purposes of
 *diversity maintenance. Values of *
 *           between 3 and 6 are useful. *
 * [objectives] - the number of objectives to the problem (fixed to 2). *
 * [residues] - the number of residues in the protein, (only integer numbers can
 *be accepted).           *
 * [archive] - the maximum number of solutions to be held in the nondominated
 *solutions archive,         *
 *             (suggested values are in the range [100,1000]). *
 * [iterations] - the program terminates after this number of iterations. *
 * [Tmax] - the program terminates after this number of fitness function
 *evaluations.                    *
 * [minmax] - set this to 0 (zero) for minimization problems, 1 for maximization
 *problems (for PSP 0)    *
 * [protein file] - input protein  instance file. *
 * [dir] - output directory for simulation results. *
 * [SS] - Secondary or Supersecondary Structure Constraints (1=secondary,
 *0=supersecondary)              *
 *                                                                                                       *
...
int seed, depth, genes, archive, objectives, iterations, Tmax,
    minmax;        // command line parameters
...
int grid_pop[MAX_LOC]; // the array holding the population residing in each grid
                       // location
      ...
      for (k = 0; k < N_fireflies; k++) {
        ...
        if (j != k) {
          ...
          if (result == -1) { // FB Dominates FA ----------> Epsilon Dominating
            ...
            // M2 = 1 + (int)(((double)genes/(double)4) *
            // exp(-(double)(2.0*num_evaluations)/(double)Tmax));

            M1 = sqrt(pow(FA->objNorm[0] - FB->objNorm[0], 2) +
                      pow(FA->objNorm[1] - FB->objNorm[1], 2)) /
                 sqrt(2);
            ...
            if (result == -1)
              cl[0] = cl[1];
            result =
                compare_min((&cl[0])->obj, (&FireflyArray[j])->obj, objectives);
...
void Normalizar(int objetivo) {
  ...
  for (int i = 0; i < N_fireflies; i++) {
    FireflyArray[i].objNorm[objetivo] =
        (-1 * (FireflyArray[i]).obj[objetivo]) / max_obj1;
    FireflyArray[i].objNorm[objetivo] =
        ((FireflyArray[i]).objNorm[objetivo] + 1) / 2;
  }
}
...
void Normalizar_sol(int i) {
  ...
  if (fabs(FireflyArray[i].obj[0]) > max_obj1) {
    ...
    FireflyArray[i].objNorm[0] = (-1 * FireflyArray[i].obj[0]) / max_obj1;
  if (fabs(FireflyArray[i].obj[1]) > max_obj2) {
    max_obj2 = fabs(FireflyArray[i].obj[1]);
    Normalizar(1);
  } else
    FireflyArray[i].objNorm[1] = (-1 * FireflyArray[i].obj[1]) / max_obj1;
}
...
int comparate_min(sol *s1, sol *s2) {
  ...
  */
  ...
  if ((s1->obj[0] <= s2->obj[0]) && (s1->obj[1] <= s2->obj[1]) &&
      ((s1->obj[0] < s2->obj[0]) || (s1->obj[1] < s2->obj[1])))
    return 1;
  return 0;
}
  ...
  if(native_ss == 1)
    {
      ...
      phi[0][0] = -67;  phi[0][1] = -47;  psi[0][0] = -57;  psi[0][1] = -37; //
H
      phi[1][0] = -130; phi[1][1] = -110; psi[1][0] = 110;  psi[1][1] = 130; //
B
      phi[2][0] = -130; phi[2][1] = -110; psi[2][0] = 110;  psi[2][1] = 130; //
E
      phi[3][0] = -59;  phi[3][1] = -39;  psi[3][0] = -36;  psi[3][1] = -16; //
G
      phi[4][0] = -67;  phi[4][1] = -47;  psi[4][0] = -80;  psi[4][1] = -60; //
I
      phi[5][0] =  -180;phi[5][1] = 180;  psi[5][0] = -180;  psi[5][1] = 180;//
T
      phi[6][0] =  -180;phi[6][1] = 180;  psi[6][0] = -180; psi[6][1] = 180; //
S
      phi[7][0] =  -180;phi[7][1] = 180;  psi[7][0] = -180; psi[7][1] = 180; //
U (undefined)
    }
  else
    {
      //Supersecondary structures constraints
      phi[0][0] = -75;  phi[0][1] = -55;  psi[0][0] = -50;  psi[0][1] = -30; //
H
      phi[1][0] = -130; phi[1][1] = -110; psi[1][0] = 110;  psi[1][1] = 130; //
E
      phi[2][0] = -150; phi[2][1] = -30;  psi[2][0] = -100; psi[2][1] = 50;  //
a
      phi[3][0] = -230; phi[3][1] = -30;  psi[3][0] = 100;  psi[3][1] = 200; //
b
      phi[4][0] =  30;  phi[4][1] = 130;  psi[4][0] = 130;  psi[4][1] = 260; //
e
      phi[5][0] =  30;  phi[5][1] = 150;  psi[5][0] = -60;  psi[5][1] = 90;  //
l
      phi[6][0] =  -160;phi[6][1] = -50;  psi[6][0] = 50;   psi[6][1] = 100; //
t
      phi[7][0] =  -180;phi[7][1] = 180;  psi[7][0] = -180; psi[7][1] = 180; //
U (undefined)
    }
  ...
  for (i = 0; i < genes; i++)
    {
      ...
      curr->chrom[i].num_angles = 2 +
get_num_sidechain_angles(curr->chrom[i].name);
    }
  fclose(fdesc);
  ...
  // initialise curr by randomly selecting the backbone and sidechain torsion
angles
...
void init_MOFA(int N_Fireflies) {
  ...
  for (i = 0; i < genes; i++) {
    ...
    if (native_ss == 1)
      ...
      curr->chrom[i].predicted = PredictedToNumber(p);
    curr->chrom[i].type = NameToType(curr->chrom[i].name);
    curr->chrom[i].num_angles =
        2 + get_num_sidechain_angles(curr->chrom[i].name);
  }
  fclose(fdesc);
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 11) {
    printf("Usage: %s [problem] [depth] [objectives] [residues] [archive] "
           "[iterations] [Tmax] [dir] [SS]\n",
           argv[0]);
    exit(-1);
  }
  ...
  signal(SIGINT,
         backup); // handler for program termination by keyboard (SIGINT signal)
  ...
  */
  ...
  /*
    ...
    num_evaluations);
        ...
        //if (i%1000==0) print_arc(i); // Uncomment for snapshot data saving
    every N iterations

        if(curr->energy < best_energy_found) best_energy_found = curr->energy;

        print_statistics("archive_statistics.dat"); // Uncomment for data saving
    of archive statistics
        print_curr_sol("current_solution_statistics.dat"); // Uncomment for data
    saving of current solution statistics
    ...
    update_grid(&cl[max_cl(cl)]);  //calculate grid location of cl[1] solution
    and renormalize archive if necessary
          archive_soln(&cl[max_cl(cl)]); //update the archive by removing all
    dominated individuals
        ...
        if (result != 1)  // if mutant is not dominated by current (else discard
    it)
          {
            if (result ==-1)  // if mutant dominates current
              {
                //printf("m dominates c\n");
                update_grid(m);           //calculate grid location of mutant
    solution and renormalize archive if necessary
                  archive_soln(m);          //update the archive by removing all
    dominated individuals

                *curr = *m;                    // replace c with m
              }
            else if(result == 0)  // if mutant and current are nondominated wrt
    each other
            {
                result = compare_to_archive(m);
                if (result != -1)  // if mutant is not dominated by archive
    (else discard it)
                      {
            update_grid(m);
            archive_soln(m);

            if((grid_pop[m->grid_loc] <= grid_pop[curr->grid_loc])||(result==1))
